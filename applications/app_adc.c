...
static THD_FUNCTION(adc_thread, arg)
{
	...
	for (;;)
	{
		...
		if (ADC_VOLTS(ADC_IND_MODE) > 1.0 && ADC_VOLTS(ADC_IND_MODE) < 2.0)
		{
			mcconf->l_current_max_scale = 0.75;
		}
		else if (ADC_VOLTS(ADC_IND_MODE) > 2.0)
		{
			mcconf->l_current_max_scale = 1.0;
		}
		else
		{
			mcconf->l_current_max_scale = 0.5;
		}
